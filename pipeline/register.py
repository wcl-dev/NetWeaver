...
_ST_PAIRS = (
    ...
    # 機關與組織用字（国台办／中央军委 這類名稱少一個字就對不上）
    "办辦 处處 厅廳 属屬 归歸 责責 务務 库庫 档檔 审審 决決 执執 监監 纪紀 检檢 组組 织織 构構 "
    "书書 岗崗 职職 单單 双雙 变變 达達 过過 还還 边邊 齐齊 丽麗 兰蘭 岁歲 万萬 亿億 图圖 画畫 "
    "杂雜 录錄 响響 誉譽 荣榮 军軍 战戰 术術 备備 护護 卫衛 击擊 侦偵 谍諜 舆輿 论論 导導 "
    # 人名與其他常見字（胡锡进／赵少康 這類已登錄實體的簡體寫法）
    "锡錫 进進 康康 荣榮 华華 强強 泽澤 东東 军軍 伟偉 杰傑 涛濤 静靜 丽麗 娟娟 芳芳 敏敏 "
    "刚剛 坚堅 显顯 详詳 简簡 洁潔 纯純 绍紹 继繼 续續 纪紀 纲綱 纬緯 绩績 缘緣 网網 罚罰"
).split()
# 刻意不收的字：里(裡/里)、后(後/后)、干(幹/乾)、面(麵/面)、松(鬆/松)、发已收發但另有髮——
# 這些一對多，在名稱比對上誤併的風險大於收益。
